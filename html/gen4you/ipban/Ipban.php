...
class Ipban {
    ...
    public function getConnection() {
        ...
        $uagent = "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:48.0) Gecko/20100101 Firefox/46.0";
        $name = $this->name;
        $ch = curl_init($name);
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 17); // таймаут соединения
        curl_setopt($ch, CURLOPT_CAINFO, dirname(__FILE__) . '/cacert.pem');
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, true);//ssl
        curl_setopt($ch, CURLOPT_TIMEOUT, 17);        // таймаут ответа
        curl_setopt($ch, CURLOPT_MAXREDIRS, 17);       // останавливаться после 10-ого редиректа
        curl_setopt($ch, CURLOPT_COOKIEFILE, $path . "\my_cookies2.txt"); //Создание файла куков
        curl_setopt($ch, CURLOPT_COOKIEJAR, $path . "\my_cookies2.txt");
...
//Создаем обьект и начинаем сериализацию
   $url = "http://www.gen4you.ml/api.php";
// старій домен $url = "http://freegen.tk/api.php";
// $url=  "http://localhost/ipban/api2.php";
